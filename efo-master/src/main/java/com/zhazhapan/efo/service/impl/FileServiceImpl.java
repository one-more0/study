...
public class FileServiceImpl implements IFileService {
    ...
    public boolean removeFile( long fileId) {
        ...
        if (Checker.isNotNull(file) && file.getIsDeletable() == 1) {
            AuthRecord authRecord = authService.getByFileIdAndUserId(fileId, 1);
            String localUrl = fileDAO.getLocalUrlById(fileId);
            isDeleted = (Checker.isNull(authRecord) ? 1 == 1 : authRecord.getIsDeletable() == 1)
                    && removeById(fileId);
            if (isDeleted) {
                FileExecutor.deleteFile(localUrl);
            }
    ...
    @Override
    public List<FileRecord> listUserDownloaded( int offset, String search) {
        return fileDAO.listUserDownloaded( offset, search);
    }

    @Override
    public List<FileRecord> listUserUploaded( int offset, String search) {
        return fileDAO.listUserUploaded( offset, search);
    }
...
//        if (user.getIsUploadable() == 1) {
            ...
            logger.info("is empty [" + multipartFile.isEmpty() + "], file size [" + size + "], max file size [" +
                    maxSize + "]");
            if (canUpload) {
                String visitUrl = getRegularVisitUrl( prefix,
                ...
                try {
                    ...
                    File file = new File(name, suffix, localUrl, visitUrl, WebUtils.scriptFilter(description),
                            WebUtils.scriptFilter(tag), 1, categoryId);
                    ...
                    if (isSuccess) {
                        long fileId = fileDAO.getIdByLocalUrl(localUrl);
                        if (fileId > 0) {
                            authService.insertDefaultAuth(1, fileId);
                        }
                    } else {
                        FileExecutor.deleteFile(localUrl);
                    }
...
//        }
    ...
    private String getRegularVisitUrl(String customUrl, String fileName, String suffix, Category category) {
        Date date = new Date();
        ...
        customUrl = customUrl.replace(YEAR, DateUtils.getYear(date)).replace(MONTH, DateUtils.getMonth(date)).replace
                (DAY, DateUtils.getDay(date)).replace(FILE_NAME, fileName)
    ...
    public boolean shareFiles(String prefix, String files, User user) {
        if (Checker.isNotEmpty(files)) {
            ...
            for (String path : paths) {
                ...
                String suffix = FileExecutor.getFileSuffix(name);
                String visitUrl = getRegularVisitUrl(prefix,  name, suffix, null);
